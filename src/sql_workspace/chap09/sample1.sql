...
--    b. 정렬을 위한 ORDER BY 절은 사용불가 (정말??) -- ORDER BY 절 가능
...
SELECT
    ...
    d.deptno
FROM
    emp e JOIN dept d       -- 자식테이블 c JOIN 부모테이블 (INNER 생략됨)
    ON e.deptno = d.deptno  --c.FK = p.PK
WHERE
    e.deptno = 20;
...
SELECT
    empno,
    ename,
    (SELECT dname FROM dept t2 WHERE t.2deptno = t1.deptno) AS dname, 
    -- 상관 부속질의 
    d.deptno
FROM
    emp e JOIN dept d
...
-- ADMIN 계정에서 권한 할당! 클라우드에서 ADMIN이 충분히 많은 권한이 있다.
-- SCOTT이 스스로에게 권한 할당하지 못한다.
...
--      b. By defauㅓlt, View 에 대해 DML 문장 실행 가능
