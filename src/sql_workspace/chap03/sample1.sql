...
SELECT
   -- || (Concatenation Operator) == concat function -> 자동형변환
   last_name || salary,
...
--     문자열의 길이 반환
-- ------------------------------------------------------
--  A. LENGTH   returns Characters <- 문자의 개수를 출력
--  B. LENGTHB  returns Bytes <- 문자를 구성하는 바이트를 출력
-- ------------------------------------------------------
SELECT
   'Oracle',
   length('Oracle')
...
SELECT
   ...
   -- substr('123456-1234567', 8),  -- 8번째 문자부터 끝까지 <- 끝까지 출력할때는 값입력X
...
-- SELECT
--    hire_date AS 입사일,
--    substr(hire_date, 8) AS 입사년도  -- 개발도구의 날짜형식에 맞춰서 사용
SELECT
   hire_date,
   to_char(hire_date) AS 입사일,
   to_char(hire_date, 'RR/MM/DD') AS 일사일2,   -- 날짜형식바꾸기 'to_char'사용하여 문자열로 변경
   to_char(hire_date, 'YYYY/MM/DD HH:MI:SS') AS 일사일3,
...
-- SYSDATE 함수: 세계표준시(UTC) 기준으로 현재날짜와 시간정보 생성
-- CUDATE_DATE 함수 : 현재 로컬 시간대(Timezone)에 맞는 날짜/시간 생성
...
-- (11) 문자(처리) 함수 - LTRIM
-- ------------------------------------------------------
--    문자열의 왼쪽에서, 지정문자 삭제(trim), 더이상 지정문자가 아니면 삭제
-- ------------------------------------------------------
SELECT
   ltrim('MMMIMLLER', 'M')
FROM
