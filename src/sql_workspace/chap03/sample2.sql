...
-- (1) 숫자 (처리)함수 - ROUND
-- ------------------------------------------------------
-- 지정한 자리 수 이하에서 반올림, 오른쪽 범위는 그자리까지 나타내라!!
-- ------------------------------------------------------

-- 1. ROUND 함수 (양수)
SELECT
    round( 456.789, 2 ) -- 소수점 2번째자리까지 나타나게 반올림
FROM
    dual;
...
-- 2. ROUND 함수 (음수)
SELECT
    round( 456.789, -1 ) -- 정수의 첫번째자리에서 반올림
FROM
    dual;
...
FROM
    employees
-- WHERE
--    sign(salary - 15000) = 1;     -- if salay > 15000
WHERE
    sign(salary - 15000) != -1;     -- if salay >= 15000
