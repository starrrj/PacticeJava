...
SELECT 
    ...
    (SALARY *(1 + * NVL(COMMISSION_PCT, 0))) * 12 AS 연봉, 
    CASE
        WHEN COMMISSION_PCT IS NULL THEN 'Salary only'         -- 조건 column
        ELSE 'Salary + Commission',    -- 비교값, 결과값
    ) AS 수당여부
FROM EMPLOYEES
ORDER BY 연봉 DESC;
...
WHERE SALARY > (
    SELECT SALARY
    FROM EMPLOYEES 
    WHERE last_name = 'Tucker'      -- 비상관부속질의
);
...
SELECT 
    ...
    ) AS "Department Avg Salary"      -- 상관부속질의
FROM 
    EMPLOYEES e
ORDER BY 
    Name ASC;
