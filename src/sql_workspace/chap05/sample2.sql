...
-- ------------------------------------------------------
-- B. Equal(= Equi) Join (동등 조인) (***) 가장많이 사용
-- ------------------------------------------------------
-- 가. 가장 많이 사용하는 조인
-- 나. 두 테이블에서, 공통으로 존재하는 컬럼의 값이 일치하는 행들을
--     연결하여 데이터를 반환. (조인조건: 병합가능해야함, Union Compatible)
--     일치하지 않는 데이터는 제외됨.
...
--  SELECT 테이블1.컬럼 , 테이블2.컬럼
--  FROM 테이블1, 테이블2
--  WHERE 테이블1.공통컬럼 = 테이블2.공통컬럼; --PK,FK로 비교연산하는 절
-- ------------------------------------------------------
SELECT
    last_name,
    ...
    department_name
FROM                    -- FROM 절은, 두개의 테이블이 지정됨 -> CP 연산수행
    employees,
    departments
WHERE
...
-- ------------------------------------------------------
-- 공통컬럼 사용시, 모호성 제거(컬럼의 모호성)
-- 테이블명.속성명 으로 모호성을 제거한다
-- ------------------------------------------------------
SELECT
    last_name,
    department_name,
    departments    -- XX 오류발생 : ORA-00918: column ambiguously defined
                   -- departments 속성이 참조하는 두 테이블 모두에게 존재함으로 테이블 중 1개로 지정해줘야함.
FROM
    employees,
    departments
WHERE
...
SELECT
    ...
    employees.department_id       -- 부서번호 , XX : 오류발생. 별칭을 사용해야한다.
FROM
...
-- 다. (*주의*) 따라서, **조인조건을 우선 명시**하고, 
--      나중에 검색조건을 명시하는 방법으로, 가독성을 향상 시켜야 한다.
...
FROM
    ...
    departments dept
WHERE
    emp.department_id = dept.department_id    -- 조인조건 (1) Join predicates
    AND last_name = 'Whalen';                 -- 검색조건 (2) Check predicates
...
FROM
    ...
    departments d
WHERE
    e.department_id = d.department_id           -- 조인조건 (1)
    AND to_char( hire_date , 'YYYY') <= 2005    -- 검색조건 (2)
GROUP BY
    -- d.department_name;
    department_name;
...
WHERE
    ...
    -- e.salary BETWEEN 1000 AND 3000; -- 범위에 속하는 직원이 겹치는 경우가 발생. 직원보다 많은 행이 추출됨
...
SELECT
    ...
    grade_level
FROM
    employees e,        -- 조인할 테이블이 N개일 때, 조인의 조건은 N-1 
    departments d,
    job_grades g
WHERE
    e.department_id = d.department_id                      -- Equal Join : 동등조인
    AND e.salary BETWEEN g.lowest_sal AND g.highest_sal;   -- Non-equal Join : 비동등조인 / 범위
...
SELECT
    ...
    *
FROM
    employees e,    -- 자식 , manager_id , fk
    employees m     -- 부모 , employee_id, pk
WHERE
...
--  SELECT 테이블1.컬럼 , 테이블2.컬럼
--  FROM 테이블1 , 테이블2
--  WHERE 테이블1.공통컬럼 = 테이블2.공통컬럼 (+); -- 왼쪽 테이블의 행을 살리게 되면서 NULL로 채워짐
-- ------------------------------------------------------
...
FROM
    ...
    employees m         -- 관리자 정보(가상)
WHERE
    e.manager_id = m.employee_id(+);       -- (+) : 왼쪽 외부조인 연산자(Oracle sql)
    e.manager_id(+) = m.employee_id;       -- (+) : 오른쪽 외부조인 연산자(Oracle sql)
    e.manager_id(+) = m.employee_id(+);    --  XX : 지원안함 
                                            --      살리는 곳 반대편, 한쪽에만 붙여라!
...
WHERE
    ...
    AND m.manager_id = mm.employee_id (+);          -- Left Outer Join
